...
if __name__ == '__main__':
    ...
    # key = 0
    print(HexBytes(HexBytes(uint32string(0))+HexBytes(uint32string(0))).hex())
    ask_string = keccak(HexBytes(uint32string(0))+HexBytes(uint32string(0)))
    print(HexBytes(ask_string).hex())
    # key = 0x29...
    ask_string = keccak(keccak(HexBytes(uint32string(0))+HexBytes(uint32string(0))))
    print(HexBytes(ask_string).hex())
